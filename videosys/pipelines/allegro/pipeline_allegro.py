...
class AllegroPABConfig(PABConfig):
    def __init__(
        ...
        spatial_threshold: list = [450, 850],
        spatial_range: int = 2,
    ):
        super().__init__(
...
class AllegroConfig:
    ...
    Args:
        ...
        pab_config (AllegroPABConfig):
            The configuration for Pyramid Attention Broadcast. Defaults to `AllegroPABConfig()`.
    ...
    Examples:
        ...
        config = AllegroConfig("Allegro", num_gpus=1)
...
class AllegroPipeline(VideoSysPipeline):
    ...
    def __init__(
        ...
        vae = AllegroAutoencoderKL3D.from_pretrained(config.model_path,subfolder='vae', torch_dtype=torch.float32).to(device)
        ...
        tokenizer = T5Tokenizer.from_pretrained(
            config.model_path,subfolder='tokenizer',
        )
    ...
    def save_video(self, video, output_path):
        ...
        # imageio.mimwrite(output_path, video, fps=15, quality=8) 
