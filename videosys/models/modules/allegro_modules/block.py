...
from videosys.models.modules.allegro_modules.rope import RoPE3D, PositionGetter3D
from videosys.models.modules.allegro_modules.embedding import CombinedTimestepSizeEmbeddings
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # attention
        if enable_pab():
            broadcast_attn, self.attn_count = if_broadcast_spatial(int(timestep_noembed.item()), self.attn_count)
        if enable_pab() and broadcast_attn:
            print(f"{int(timestep_noembed.item())} : accelerated")
